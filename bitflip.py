...
def flip_bit(tensor, ind):
    bit_float = float2bit(tensor)
    ...
    bit_float[ind] = flip
    return bit2float(bit_float)   
    
def flip_random_bit(tensor):
    # get random bit to flip
    bit_float = float2bit(tensor)
    rand_ind = random.randint(0, bit_float.shape[0] - 1)
    
    bit = bit_float[rand_ind].item()
    if bit == 0:
        flip = 1.
    else:
        flip = 0.
    
    bit_float[rand_ind] = flip
    return bit2float(bit_float)    